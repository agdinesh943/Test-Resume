...
window.addEventListener('DOMContentLoaded', function () {
    ...
    function createProjectGroup(index) {
        ...
        group.innerHTML = `
            ...
            <div class="form-group">
                <label for="projectDescription${index}">Project Description (10-35 words)</label>
                <textarea id="projectDescription${index}" name="projectDescription${index}" rows="3"></textarea>
                <div class="char-counter" id="pd${index}Counter">0/35 words</div>
            </div>
...
function initProjectDescriptionCounters() {
    console.log('Initializing project description and internship word counters...');
    const inputs = [
        { id: 'projectDescription1', counter: 'pd1Counter', max: 35, min: 10 },
        { id: 'projectDescription2', counter: 'pd2Counter', max: 35, min: 10 },
        { id: 'projectDescription3', counter: 'pd3Counter', max: 35, min: 10 },
        { id: 'jobDescription', counter: 'jobDescriptionCounter', max: 35, min: 10 }
    ];
...
function validateForm(formData) {
    ...
    const descriptionFields = [
        { field: 'projectDescription1', name: 'Project 1 Description' },
        { field: 'projectDescription2', name: 'Project 2 Description' },
        { field: 'projectDescription3', name: 'Project 3 Description' },
        { field: 'jobDescription', name: 'Internship Description' }
    ];

    for (let { field, name } of descriptionFields) {
        if (formData[field]) {
            const words = formData[field].trim().split(/\s+/).filter(word => word.length > 0);
            if (words.length < 10) {
                alert(`${name} must have at least 10 words. Currently has ${words.length} words.`);
                return false;
            }
            if (words.length > 35) {
                alert(`${name} must not exceed 35 words. Currently has ${words.length} words.`);
                return false;
            }
        }
    }
